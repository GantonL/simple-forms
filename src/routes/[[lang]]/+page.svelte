...
<BasePage title="common.brand.name" description="seo.pages.home.description">
	...
	<div class="fixed inset-0 -z-10 h-full w-full bg-white dark:bg-slate-950">
		...
		></div>
		<div
			class="bg-primary/20 absolute top-0 right-0 left-0 -z-10 m-auto h-[310px] w-[310px] rounded-full opacity-20 blur-[100px]"
		></div>
	</div>

	<div class="flex justify-center overflow-hidden px-4">
		<div class="flex w-full max-w-6xl flex-col gap-24 pt-10 pb-20 md:gap-32 md:pt-20">
			<!-- Hero Section -->
			<section class="relative flex flex-col items-center gap-8 text-center">
				{#if mounted}
					...
					>
						...
						>
							<span
								class="from-primary to-primary/60 bg-gradient-to-r bg-clip-text text-transparent"
							>
								{$t('landing.hero.headline')}
							</span>
						</h1>

						<p class="text-muted-foreground max-w-2xl text-xl md:text-2xl">
						...
						<div class="mt-4 flex flex-col gap-4 sm:flex-row">
							<Button
								...
								class="hover:shadow-primary/25 h-12 px-8 text-lg font-semibold shadow-lg transition-all hover:-translate-y-1"
							>
							...
							<Button
								...
								class="hover:bg-secondary/50 h-12 px-8 text-lg"
							>
					...
					{#each features as feature, i (feature.title)}
						{#if mounted}
							<div in:fly={{ y: 20, duration: 500, delay: 200 + i * 100 }}>
								<Card.Root
									class="group border-muted/50 hover:border-primary/20 hover:shadow-primary/5 h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
								>
									<Card.Header>
			...
			<section class="bg-muted/30 relative overflow-hidden rounded-3xl border px-6 py-16 md:px-12">
				<div
						...
						{#each steps as step, i (step.number)}
							{#if mounted}
								...
								>
									...
									{#if i !== steps.length - 1}
										<div
											class="bg-border absolute top-8 -z-10 hidden h-[2px] w-[calc(100%+2rem)] md:block ltr:left-[50%] rtl:right-[50%]"
										></div>
									{/if}

									<div
										class="bg-background ring-border group-hover:shadow-primary/20 group-hover:ring-primary flex h-16 w-16 items-center justify-center rounded-2xl shadow-lg ring-1 transition-all duration-300 group-hover:scale-110"
									>
										<span class="text-primary text-2xl font-bold">{step.number}</span>
									</div>
									<div class="bg-background/50 flex flex-col gap-2 p-4 backdrop-blur-sm">
					...
					{#each benefits as benefit, i (benefit.title)}
						{#if mounted}
							<div in:fly={{ y: 20, duration: 500, delay: 600 + i * 100 }}>
								<div
									class="bg-card hover:border-primary/20 flex h-full flex-col gap-3 rounded-2xl border p-6 shadow-sm transition-all duration-300 hover:shadow-md"
								>
									<div
										class="bg-primary/10 text-primary mb-2 flex h-12 w-12 items-center justify-center rounded-lg"
									>
			...
			<section>
				{#if mounted}
					<div
						...
						class="bg-primary text-primary-foreground relative flex flex-col items-center gap-8 overflow-hidden rounded-3xl px-6 py-20 text-center shadow-2xl md:px-12"
					>
						<div
						...
						></div>
						<div
							class="absolute -top-20 -left-20 h-64 w-64 rounded-full bg-white/10 blur-3xl"
						></div>
						<div
						...
						<p class="text-primary-foreground/80 max-w-2xl text-lg md:text-xl">
